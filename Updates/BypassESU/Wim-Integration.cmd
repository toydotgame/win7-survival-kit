...
set "_EsuCom=amd64_microsoft-windows-s..edsecurityupdatesai_31bf3856ad364e35_6.1.7602.20587_none_c8993b883659a816"
set "_EsuIdn="
set "_EsuHsh=E84B2D35DAC9DAC47C709405E4E64B007D3E9D93F6B9C4E2EE79BFB4CDC07FF8"
) else (
set "xOS=x86"
set "xPA=x86"
set "xBE=bbe32.exe"
...
set "_EsuCom=x86_microsoft-windows-s..edsecurityupdatesai_31bf3856ad364e35_6.1.7602.20587_none_6c7aa0047dfc36e0"
set "_EsuIdn="
set "_EsuHsh=F27DFF1EBCE8D8CC1593A62C772D84F0245BEDC9672CB71833D302A8F334901F"
)
set "_PKG=Package_for_KB4528069~31bf3856ad364e35~%xPA%~~6.1.1.1"
...
if %shaupd% neq 1 goto :E_SSUSHA
if %ssuver% equ 0 goto :E_SSUSHA
if %ssuver:~4,4% equ 7601 if %ssuver:~9,5% lss 24554 goto :E_SSUSHA

set _Suppt=1

set _SrvrC=0
if exist "%target%\Windows\WinSxS\Manifests\%xPA%_windowsserverfoundation_*.manifest" set _SrvrC=1&set _Suppt=0

set _Embed=0
if exist "%target%\Windows\Servicing\Packages\*Winemb-*.mum" set _Embed=1&set _Suppt=0

set _WinPE=0
if exist "%target%\Windows\Servicing\Packages\*WinPE-LanguagePack*.mum" set _WinPE=1&set _Suppt=0
...
set _EsuPkg=0
if %_Suppt% equ 1 if exist "%target%\Windows\Servicing\Packages\%_PKG%.mum" (
reg query "%_CBS%\Packages\%_PKG%" /v CurrentState 2>nul | find /i "0x70" 1>nul && (
  reg query "%_EsuKey%\6.1" /ve 2>nul | find /i "%_OurVer%" 1>nul && set _EsuPkg=1
  )
)
...
if %_Suppt% equ 1 call :IMGc
if %_Suppt% equ 0 call :IMGt 1>nul 2>nul
...
echo.            ^<assemblyIdentity  name="Package_for_KB4528069" version="6.1.1.1" language="neutral" processorArchitecture="%xPA%" publicKeyToken="31bf3856ad364e35" /^>
echo.            ^<source location="Windows6.1-KB4528069-%xOS%.cab" /^>
echo.        ^</package^>
echo.     ^</servicing^>
echo.^</unattend^>
)>Stage.xml
...
dism.exe /Image:"%target%" /Apply-Unattend:Stage.xml
if %errorlevel% neq 0 if %errorlevel% neq 3010 goto :E_PKG
1>nul 2>nul reg load HKLM\wSOFTWARE "%target%\Windows\System32\Config\SOFTWARE"
1>nul 2>nul reg add "%_EsuKey%" /f /ve /d %_OurVer:~0,3%
1>nul 2>nul reg add "%_EsuKey%\%_OurVer:~0,3%" /f /ve /d %_OurVer%
1>nul 2>nul reg add "%_EsuKey%\%_OurVer:~0,3%" /f /v %_OurVer% /t REG_BINARY /d 01
1>nul 2>nul reg unload HKLM\wSOFTWARE
dism.exe /Image:"%target%" /Apply-Unattend:Install.xml
if %errorlevel% neq 0 if %errorlevel% neq 3010 goto :E_PKG
del /f /q Stage.xml Install.xml 1>nul 2>nul
exit /b
...
echo.
if %_Suppt% equ 1 dism.exe /Image:"%target%" /NoRestart /Remove-Package /PackageName:%_PKG%
if %errorlevel% neq 0 if %errorlevel% neq 3010 (
reg load HKLM\wSOFTWARE "%target%\Windows\System32\Config\SOFTWARE" 1>nul 2>nul
reg add "%_CBS%" /f /v Clean /t REG_DWORD /d 1 1>nul 2>nul
reg unload HKLM\wSOFTWARE 1>nul 2>nul
echo.
echo %_err%
echo Failed removing KB4528069 Package.
echo this may occur if you have ESU updates integrated.
goto :TheEnd
)
...
:E_PKG
del /f /q Stage.xml Install.xml 1>nul 2>nul
dism.exe /Image:"%target%" /NoRestart /Remove-Package /PackageName:%_PKG% 1>nul 2>nul
reg load HKLM\wSOFTWARE "%target%\Windows\System32\Config\SOFTWARE" 1>nul 2>nul
reg add "%_CBS%" /f /v Clean /t REG_DWORD /d 1 1>nul 2>nul
reg unload HKLM\wSOFTWARE 1>nul 2>nul
echo.
echo %_err%
echo Failed Integrating KB4528069 Package.
echo discard the image and try again.
echo.
...
:E_SSUSHA
echo %_err%
echo Required updates are missing. You must integrate:
echo 1) KB4490628
echo 2) KB4474419
echo 3) KB4550735 [Mar 2020] or later servicing stack update
goto :TheEnd
