...
set _SrvrC=0
wimlib-imagex dir "%target%" 1 --path=Windows\Servicing\Packages 2>nul | find /i "Microsoft-Windows-ServerCore-Package" 1>nul && set _SrvrC=1&set _Suppt=0

set _Embed=0
wimlib-imagex dir "%target%" 1 --path=Windows\Servicing\Packages 2>nul | find /i "WinEmb-" 1>nul && set _Embed=1&set _Suppt=0

set _WinPE=0
wimlib-imagex dir "%target%" 1 --path=sources\recovery\RecEnv.exe 1>nul 2>nul && set _WinPE=1&set _Suppt=0
...
reg add "%IFEO%\msiexec.exe" /f /v GlobalFlag /t REG_DWORD /d 256
reg unload HKLM\wSOFTWARE
type nul>wimupdt.txt
