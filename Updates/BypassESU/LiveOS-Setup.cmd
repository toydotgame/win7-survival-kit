...
set _SrvrC=0
if exist "%SystemRoot%\WinSxS\Manifests\%xPA%_windowsserverfoundation_*.manifest" set _SrvrC=1&set _Suppt=0

set _Embed=0
if exist "%SystemRoot%\Servicing\Packages\*Winemb-*.mum" set _Embed=1&set _Suppt=0
...
set _EsuPkg=0
if %_Suppt% equ 1 if exist "%SystemRoot%\Servicing\Packages\%_PKG%.mum" (
...
if %_Suppt% equ 1 call :DoInstall
if %_Suppt% equ 0 call :DoManual 1>nul 2>nul
...
dism.exe /Online /Apply-Unattend:Stage.xml
...
dism.exe /Online /Apply-Unattend:Install.xml
if %errorlevel% neq 0 if %errorlevel% neq 3010 goto :E_PKG
...
echo.
if %_Suppt% equ 1 dism.exe /Online /NoRestart /Remove-Package /PackageName:%_PKG%
...
echo this may occur if you have ESU updates installed.
goto :TheEnd
)
...
:E_PKG
del /f /q Stage.xml Install.xml 1>nul 2>nul
dism.exe /Online /NoRestart /Remove-Package /PackageName:%_PKG% 1>nul 2>nul
reg add "%_CBS%" /f /v Clean /t REG_DWORD /d 1 1>nul 2>nul
echo.
echo %_err%
echo Failed installing KB4528069 Package.
...
echo 1) KB4490628
echo 2) KB4474419
echo 3) KB4555449 [May 2020] or later servicing stack update
goto :TheEnd
